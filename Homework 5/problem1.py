...
def compute_L(z, y):
    #########################################
    ## INSERT YOUR CODE HERE (5 points)
    L = z*(1-y) if z>100 else np.log(1+np.exp(z)) - y*z
    #########################################
    return L
    #-----------------
    '''  e
...
def backward(x, y, z):
    ...
    dL_dw, dL_db = (compute_dL_dz(z,y) * compute_dz_dw(x)), (compute_dL_dz(z,y) * compute_dz_db())
...
def train(X, Y, alpha=0.001, n_epoch=100):
    ...
    for _ in range(n_epoch): # iterate through the dataset n_epoch times
        ...
        for i in indices: # iterate through each random training sample (x,y)
            ...
            dL_dw = compute_dL_dw(compute_dL_dz(compute_z(x,w,b),y), compute_dz_dw(x))
            dL_db = compute_dL_db(compute_dL_dz(compute_z(x,w,b),y), compute_dz_db())
...
def predict(x, w, b):
    #########################################
    ## INSERT YOUR CODE HERE (5 points)
    y = 0 if compute_z(x, w, b) <=0 else 1
    #########################################
    return y
    #-----------------
    '''  

...
def filter_height(X, t):
    #########################################
    ## INSERT YOUR CODE HERE (3 points)
    Xt = X[X['height']>=t].head() #what we do here is find the X - dataframe that has height greater
    # than or equal to and call those dataframe arguments
    #########################################
    return Xt
    #-----------------
    '''  
...
def group_sum(X, k):
    #########################################
    ## INSERT YOUR CODE HERE (3 points)
    Y = X.groupby([k]).sum() # group by k, take the sum
...
def merge(X, Y, k):
    #########################################
    ## INSERT YOUR CODE HERE (3 points)
    J = X.merge(Y, on = [k]) # merge x with y on the fields equalling k
    #########################################
    return J
    #-----------------
    '''  
...
def sort_values(X, k):
    #########################################
    ## INSERT YOUR CODE HERE (3 points)
    Y = X.sort_values(by = [k], ascending = False) # sort by which column, in descending
    #########################################
    return Y
    #-----------------
    '''  
...
def insert_column(X, y, k):
    #########################################
    ## INSERT YOUR CODE HERE (3 points)
    X.insert(len(X.columns),k,y, allow_duplicates = False) #loc works with 0 too
    #########################################
    #-----------------
    '''  
        TEST: Now you can test the correctness of your code above by typing the following in the terminal:

...
def remove_sink_nodes(A):
    ...
    holder = np.all(A==0, axis = 0) # this can be put directly into len and in the if statement, but this is cleaner

    for eachElement in range(len(holder)):

        if holder[eachElement]:
...
def add_column_pagerank(X2, x):
    ...
    X2.insert(len(X2.columns), 'PageRank', x, allow_duplicates = False)
